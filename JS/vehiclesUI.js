...
function vehiclesUI()
{	console.log("the vehicles UI is running");
	Lheader("images/raw/vehiclestitle.png");
	Lbodyvehicles("images/raw/vehicles/boat.png", "images/raw/vehicles/1.png");
	Lfooter();
}
...
function Lbodyvehicles(objimage, objmaindisplay)
{	
...
function mySlidevehicles(param)
{
	if(param === 'next')
       {
                  i++;
                  if(i === imagesrawvehicles.length){ i = imagesrawvehicles.length - 1; }
         }
	else{
                  i--;
                  if(i < 0){ i = 0; }
    ...
    document.getElementById('slide').src = vehiclesImages[i];
    document.getElementById('slide1').src = imagesrawvehicles[i];
}