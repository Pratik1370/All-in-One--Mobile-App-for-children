...
function monthsUI()
{	console.log("the months UI is running");
	Lheader("images/raw/monthstitle.png");
	Lbodymonths("images/raw/months/jan.png", "images/raw/months/1.png");
	Lfooter();
}
...
function Lbodymonths(objimage, objmaindisplay)
{	
...
function mySlidemonths(param)
{
	if(param === 'next')
       {
                  i++;
                  if(i === imagesrawmonths.length){ i = imagesrawmonths.length - 1; }
         }
	else{
                  i--;
                  if(i < 0){ i = 0; }
    ...
    document.getElementById('slide').src = monthsimages[i];
    document.getElementById('slide1').src = imagesrawmonths[i];
}