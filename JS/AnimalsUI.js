...
function AnimalsUI()
{	i=0;
	console.log("the animals UI is running");
	Lheader("images/raw/animalstitle.png");
	Lbodyanimals("images/raw/alphabets/cat.png", "images/raw/animals/1.png");
	Lfooter();
}
//defines the body of the animalsUI
function Lbodyanimals(objimage, objmaindisplay)
{	
...
function mySlideanimals(param)
{
	if(param === 'next')
       {
                  i++;
                  if(i === imagesrawanimals.length){ i = imagesrawanimals.length - 1; }
         }
	else{
                  i--;
                  if(i < 0){ i = 0; }
    ...
    document.getElementById('slide').src = animalsimages[i];
    document.getElementById('slide1').src = imagesrawanimals[i];
}