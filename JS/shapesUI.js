...
{	
	i=0;
	console.log("the shapes UI is running");
	Lheader("images/raw/shapestitle.png");
	Lbodyshapes("images/raw/shapes/line.png", "images/raw/shapes/1.png");
	Lfooter();
}
...
function Lbodyshapes(objimage, objmaindisplay)
{	
...
function mySlideshapes(param)
{
	if(param === 'next')
       {
                  i++;
                  if(i === imagesrawshapes.length){ i = imagesrawshapes.length - 1; }
         }
	else{
                  i--;
                  if(i < 0){ i = 0; }
    ...
    document.getElementById('slide').src = shapesImages[i];
    document.getElementById('slide1').src = imagesrawshapes[i];
}