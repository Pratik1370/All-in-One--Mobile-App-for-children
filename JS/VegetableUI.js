var i=0; //used for changing the alphabets position
//fetches all data from the UI of learningtools
function vegetablesUI()
{	console.log("the vegetbale UI is running");
	Lheader("images/raw/vegetablesTitle.png");
	Lbodyvegetable("images/raw/Vegetables/capsicum.png", "images/raw/vegetables/1.png");
	Lfooter();
}
...
function Lbodyvegetable(objimage, objmaindisplay)
{	
...
function mySlidevegetable(param)
{
	if(param === 'next')
       {
                  i++;
                  if(i === imagesrawvegetables.length){ i = imagesrawvegetables.length - 1; }
         }
	else{
                  i--;
                  if(i < 0){ i = 0; }
    ...
    document.getElementById('slide').src = vegetableimages[i];
    document.getElementById('slide1').src = imagesrawvegetables[i];
}